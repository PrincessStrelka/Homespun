...
public class colourHandler{
  ...
  static void registerBlockColours(RegisterColorHandlersEvent.Block event){
    event.register((blockState, getter, blockPos, tintIndex) -> getter != null && blockPos != null ? BiomeColors.getAverageFoliageColor(getter, blockPos) : FoliageColor.getDefaultColor(), ModBlocks.IRONWOOD_LEAVES.get());
  ...
  public static void registerItemColors(RegisterColorHandlersEvent.Item event){
    ...
    event.register((stack, tintIndex) -> {if(stack.getItem() instanceof BlockItem blockItem) return blockColors.getColor(blockItem.getBlock().defaultBlockState(), null, null, tintIndex);return 0xFFFFFF;}, ModBlocks.IRONWOOD_LEAVES.get());
